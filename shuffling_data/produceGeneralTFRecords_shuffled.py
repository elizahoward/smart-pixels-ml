...
from . import OptimizedDataGenerator4_shuffled as ODG

# Make general tf records directory
batch_size = 2048
directory_name = f'filtering_records{batch_size}_shuffled'
...
print(f"Creating shuffled TF records with seed {random_seed}")
print(f"Training files: {file_count}")
print(f"Validation files: {total_files-file_count}")

start_time = time.time()
training_generator = ODG.OptimizedDataGeneratorShuffled(
                ...
                tf_records_dir = str(tf_dir_train),  # Convert Path to string
...
start_time = time.time()
validation_generator = ODG.OptimizedDataGeneratorShuffled(
                data_directory_path = data_directory_path,
                is_directory_recursive = is_directory_recursive,
                file_type = file_type,
                data_format = data_format,
                ...
                transpose = transpose,
                time_stamps = time_stamps,
                tf_records_dir = str(tf_dir_validation),  # Convert Path to string
                x_feature_description=x_feature_description,
                filteringBIB=filteringBIB,
                shuffle_data=shuffle_data,
...
print(f"TF records created successfully in {records_dir}")
