...
class OptimizedDataGeneratorShuffled(tf.keras.utils.Sequence):
    def __init__(self, 
        ...
        This version includes proper shuffling between signal and BIB data during TF record creation.
        ...
        shuffle_data: Whether to shuffle signal and BIB data during TF record creation.
        ...
        # otherwise write the tf record
            ...
            # SHUFFLE THE FILE ORDERING if requested
            if shuffle_data:
                print(f"Shuffling signal and BIB files with seed {random_seed}")
                # Create pairs of (file, type) for shuffling
                bib_files_with_type = [(f, 'bib') for f in self.recon_files_bib_subset]
                sig_files_with_type = [(f, 'sig') for f in self.recon_files_sig_subset]
                
                # Combine and shuffle
                all_files_with_type = bib_files_with_type + sig_files_with_type
                random.shuffle(all_files_with_type)
                
                # Separate back into file lists
                self.recon_files = [f[0] for f in all_files_with_type]
                self.file_types = [f[1] for f in all_files_with_type]
                
                # Print shuffling statistics
                print(f"Total files: {len(self.recon_files)} (BIB: {self.file_types.count('bib')}, SIG: {self.file_types.count('sig')})")
                
                # Calculate shuffling metrics
                alternations = 0
                max_run = 1
                current_run = 1
                
                for i in range(1, len(self.file_types)):
                    if self.file_types[i] != self.file_types[i-1]:
                        alternations += 1
                        current_run = 1
                    else:
                        current_run += 1
                ...
                print(f"  - Alternation rate: {alternations/(len(self.file_types)-1)*100:.1f}%")
                
                # Show first 20 file types
                print(f"First 20 file types: {self.file_types[:20]}")
                
            else:
                # Original behavior: BIB first, then signal
                self.recon_files = self.recon_files_bib_subset + self.recon_files_sig_subset
                self.file_types = ['bib'] * len(self.recon_files_bib_subset) + ['sig'] * len(self.recon_files_sig_subset)
            ...
            print(f"Processing {len(self.recon_files)} files (shuffled: {shuffle_data})")
            
            for i, file in enumerate(self.recon_files):
                file_type_indicator = self.file_types[i] if shuffle_data else ('sig' if 'sig' in file else 'bib')
                print(f"Processing file {i+1}/{len(self.recon_files)}: {os.path.basename(file)} ({file_type_indicator})")
