...
def build_custom_cnn_model(x_profile_length=21, z_global_length=1, y_profile_length=13, y_local_length=1, dropout_rate=0.2):
    """
    ...
    # x_profile + z_global branch (single 128-unit layer)
    xz_concat = Concatenate(name="xz_concat")([x_profile_input, z_global_input])
    xz_dense = Dense(128, activation="relu", name="xz_dense1")(xz_concat)

    # y_profile + y_local branch (single 128-unit layer)
    yl_concat = Concatenate(name="yl_concat")([y_profile_input, y_local_input])
    yl_dense = Dense(128, activation="relu", name="yl_dense1")(yl_concat)
