...
This file:
1. Loads a pre-trained model (or trains one if not available)
...
MODIFIED VERSION: Can load a saved model to avoid retraining.
"""
...
import os
import sys
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '../../../smart_pixels_ml')))
...
def export_quantized_weights_to_csv(quantized_weights, bits, output_prefix):
    """
    Export quantized weights and biases to a CSV file.
    Each row: layer, type (kernel/bias), index, value, bits
    """
    rows = []
    for name, arr in quantized_weights.items():
        ...
        for idx, val in enumerate(flat):
            rows.append({'layer': layer, 'type': ltype, 'index': idx, 'value': val, 'bits': bits})
    df = pd.DataFrame(rows)
    csv_path = f"{output_prefix}_quantized_{bits}bit_weights.csv"
    df.to_csv(csv_path, index=False)
    print(f"Quantized weights ({bits}-bit) exported to: {csv_path}")
...
def main():
    ...
    try:
        ...
        # Step 6: Export quantized weights for all bitwidths to CSV
        output_prefix = "quantized_model"
        for bits in [16, 8, 4, 2]:
            quantized_weights = quantize_model_weights_and_biases(model, bits=bits)
            export_quantized_weights_to_csv(quantized_weights, bits, output_prefix)
        ...
        print("7. ✅ Quantized weights exported to CSV")
